...
class Patient(object):
    ...
    def update(self):
        ...
        #Determine if each virus particle survives:
        virusListCopy = self.viruses[:]
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for t in range(300):
            patient.update()
            allPoints[t].append(patient.getTotalPop())
    #print(allPoints)
    avgVirusList = []
    timeStepList = []
    for i in range(300):
        timeStepList.append(i)
        avgVirus = sum(allPoints[i])/len(allPoints[i])
        avgVirusList.append(avgVirus)
    #print(timeStepList,avgVirusList)
    pylab.figure("Simulation without Drug")
    pylab.xlabel("time")
    pylab.ylabel("average number of viruses")
    #pylab.figure("Plot of simulation without drug")
    pylab.plot(timeStepList,avgVirusList,label = "without drug")
    pylab.legend(loc = 'upper left')
    pylab.title("Plot of simulation without drug")
    pylab.show()
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in virusListCopy:
            try:
                newVirus = virus.reproduce(popDensity,self.activeDrugs)
                if newVirus:
                    self.viruses.append(newVirus)
                popDensity = self.getTotalPop() / self.getMaxPop()
            except NoChildException:
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for t in range(300):
            ...
            patient.update()
            allPoints[t].append(patient.getTotalPop())
            allGuttagonolPoints[t].append(patient.getResistPop(['guttagonol']))
    ...
    for i in range(300):
        ...
        avgVirus = sum(allPoints[i])/len(allPoints[i])
        avgVirusList.append(avgVirus)
        guttagonol = sum(allGuttagonolPoints[i])/len(allGuttagonolPoints[i])
        guttagonolList.append(guttagonol)
    #print(timeStepList,avgVirusList)
    pylab.figure("Simulation with Drug")
    pylab.xlabel("time")
    pylab.ylabel("average number of viruses")
    pylab.plot(timeStepList,avgVirusList,label = "total viruses")
    pylab.plot(timeStepList,guttagonolList,label = "guttagonol resistant viruses")
    pylab.legend(loc = 'upper left')
    pylab.title("Plot of simulation with drug")
    pylab.show()
