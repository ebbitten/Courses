...
def generate_models(x, y, degs):
    ...
    returnList=[]
    for deg in degs:
        bestFit = np.polyfit(xNump,yNump,deg)
        # print(bestFit)
        returnList.append(bestFit)
        #print(deg,degs,"these are the degs")
    #print(returnList)
    return(returnList)
...
def r_squared(y, estimated):
    ...
    Args:
        ...
        estimated: a list of values ` by the regression model
    Returns:
    ...
    for i in range(len(y)):
        sumEstimated += (y[i]-estimated[i])**2
        sumMean += (y[i]-mean)**2
    return (1-(sumEstimated/sumMean))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        for i in range(len(x)):
            yEst.append(np.polyval(model,x[i]))
    r2 = r_squared(y,yEst)
    print(r2)
