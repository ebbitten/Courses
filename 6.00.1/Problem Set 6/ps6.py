...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        upperX2=upper+upper
        for i in lower:
            dict1[i] = lowerX2[shift+lower.index(i)]
        for i in upper:
            dict1[i] = upperX2[shift+upper.index(i)]
        return dict1
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            correctWords=0
            textTry=self.apply_shift(shift)
            words=textTry.split()
            for word in words:
                if word in self.valid_words:
                    correctWords+=1
            if correctWords>bestWords:
                bestShift=shift
                bestTry=textTry
                bestWords=correctWords
        return (bestShift,bestTry)

def decrypt_story():
    story=CiphertextMessage(get_story_string())
    decrypted=story.decrypt_message()
    # print(decrypted)
    # print(decrypted[1])
    return(decrypted[1])
